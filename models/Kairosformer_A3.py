...
class Model(nn.Module):
    """
    A3: v0 + AutoCorrelation everywhere (encoder & decoder)
    """
    def __init__(self, configs):
        ...
        # Encoder: AutoCorrelation inside Transformer encoder block
        self.encoder = Transformer_Encoder(
            [
                Transformer_EncoderLayer(
                    AutoCorrelationLayer(
                        AutoCorrelation(
                            mask_flag=False,                   # encoder self-attn is non-causal
        ...
        # Decoder: AutoCorrelation for both self- and cross-attn
        def _ac_self():
