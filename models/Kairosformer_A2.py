...
class Model(nn.Module):
    """
    A2: v0 + AutoCorrelation in decoder only (encoder stays ProbSparse)
    """
    def __init__(self, configs):
        ...
        # Encoder = SAME as v0: ProbSparse (via AttentionLayer/AutoCorrelationLayer wrapper used in v0’s encoder)
        ...
        # Decoder: AutoCorrelation in BOTH self- and cross-attention
        def _ac_self():
            return AutoCorrelationLayer(
                AutoCorrelation(
                    mask_flag=True,                    # causal for decoder self-attn
        ...
        def _ac_cross():
            return AutoCorrelationLayer(
                AutoCorrelation(
                    mask_flag=False,                   # non-causal for cross-attn
                    factor=configs.factor,
                    attention_dropout=configs.dropout,
                    output_attention=False,
                ),
        ...
        self.decoder = Decoder(
            [
                DecoderLayer(
                    self_attention=_ac_self(),
                    cross_attention=_ac_cross(),
                    d_model=configs.d_model,
                    c_out=configs.c_out,
                    d_ff=configs.d_ff,
